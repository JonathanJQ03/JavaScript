...
//generar una funcion que escriba una id para una ip:
//recordando que una ip va del 0 al 255 y son 4 bloques separados por un punto
//para evitar problemas con la numeracion octal debemos evitar el 0 como valor dentro del ejercicio
let generarIp = () => {
    let cadena = "";
    let generador = 0;
    while(generador < 4){
        let numeroAletario = Math.random();
        numeroAletario = numeroAletario*1000;
        //recordar que puedo usar el math.floor para redondear hacia abajo
        //del mismo modo multiplicar por 256 hace q el maximo valor sea de 255 por reglas de decimales hasta el 0.999 periodico
        numeroAletario = numeroAletario.toFixed(0);
        if(numeroAletario >= 0 && numeroAletario <= 255){
            cadena = `${numeroAletario}.${cadena}`;
            generador++;
        }
       
    }
    //para poder eliminar el punto que queda al final de la cadena simplemente pondria el valor -1
    //De esta mamera se avanza continuamente en la cadena 
...
//Hola Javier de las 10pm taba dura la chamba jajaj pero ya acabaste pondraste a acabar hoy funciones porfa
//no andaras jeugando cod ya nos vemos en la noche para mejorar este codiguin
